...
template_conf_directory = os.path.join(USER_DIRECTORY,
                                       KEY_TEMPLATE_DIRECTORY)
...
template_conf_image_no_diagram = os.path.join(USER_DIRECTORY,
                                              KEY_TEMPLATE_NO_IMAGE)

try:
    os.makedirs(template_conf_directory)
except:
    pass
...
def DialogChooserBackup(mode):
    ...
    if mode == 'save':
        ...
        chooser.set_current_folder(os.path.join(INSTALLATION_DIRECTORY,
                                                KEY_TEMPLATE_DIRECTORY))
...
def RestoreBackupConfiguration():
    ...
    if response and os.path.isfile(filename):
        try:
            ConfigObj(filename, raise_errors = True)
...
class GlobalConfiguration:
    ...
    def AddDiagram(self,name,diagram):
        # diagram here is not an instance of DiagramConfiguration,
        # it's just a dict
    ...
    def BuildFromDiagram(self, diag):
        ...
        for node_name in Nodes.sections:
            ...
            if tex_node == '':
                s += " " * (rientro+2) + "\\node (%s) at (%d, %d) {};\n" % (self.commu.GenerateNodeName(r, c), c, r)
            else:
                s += " " * (rientro+2) + "\\node (%s) at (%d, %d) {$%s$};\n" % (self.commu.GenerateNodeName(r, c), c, r, tex_node)
        ...
        for arr in Arrows.sections:
            ...
            coda =   int(arrow.get(key_arrows_coda,
                                   default_key_arrows_coda))
            testa =  int(arrow.get(key_arrows_testa,
                                   default_key_arrows_testa))
            inarcamento = int(arrow.get(key_arrows_inarcamento,
                                        default_key_arrows_inarcamento))
            altobasso = int(arrow.get(key_arrows_altobasso,
                                      default_key_arrows_altobasso))
            decorazione = int(arrow.get(key_arrows_decorazione,
                                        default_key_arrows_decorazione))
    ...
    def CreateImageForDiagram(self,
                              diagram,
                              remove_temp = False,
                              add_image = True,
                              name = None):
        ...
        try:
            ...
            ConvertProc = subprocess.Popen(args = args,
            ...
            if name:
                ...
                try:
                    shutil.move(image_output,new_image_output)
                except:
                    raise Exception
    ...
    def SetDiagramInGui(self, diag, diag_name):
        ...
        for arr in Arrows.sections:
            ...
            decorazione = int(arrow.get(key_arrows_decorazione,
                                        default_key_arrows_decorazione))
            tipo = int(arrow.get(key_arrows_preset,
    ...
    def CreateBackupConfiguration(self, widget = None, data = None):
        ...
        if response:
            try:
                shutil.copy(template_conf_file,filename)
            except OSError, Err:
                display_warning('Unsuccessful creation of a backup due to the following error:\n%s' % str(Err))

    def RestoreBackupConfigurationCallback(self,
                                           widget = None,
                                           data = None):
        if not RestoreBackupConfiguration():
            return
        self.DeleteLoadWindow(None)
    ...
    def LoadConfiguration(self):
        ##### Read Configuration ######
        self.conf = ConfigObj(template_conf_file,
                              raise_errors = True,
                              create_empty = True)
    ...
    def SaveDirectlyToTheLastDiagram(self):
        if not (hasattr(self,'last_diagram_saved') and \
                    self.last_diagram_saved in self.diagrams.sections):
            self.CreateChooseNameWindow()
            return

        new_diagram = self.LoadDiagramFromGui()
        self.CreateImageForDiagram(new_diagram,
                                   name = self.last_diagram_saved,
                                   remove_temp = True)
        old_image = self.diagrams[self.last_diagram_saved][key_diagram_image]
        if not old_image in \
                [new_diagram.conf[key_diagram_image],
    ...
    def DeleteChooseNameWindow(self, widget, data = None):
        if not self.renaming:
            self.preview_for_template.RemoveTemporaryFiles()
    ...
    def CreateChooseNameWindow(self,
                               ...
                               old_name = ''):
        ## in the rename mode new_diagram is not a
        ## DiagramConfiguration istance
        ...
        button = GetButtonFromStock(gtk.STOCK_CANCEL,
                                    'Cancel',
                                    self.DeleteChooseNameWindow)
        hbox.pack_end(button, expand = False, fill = False)

        ## Save button
        button = GetButtonFromStock(gtk.STOCK_SAVE,
                                    'Save',
                                    self.SaveChooseNameWindow,
                                    old_name)
        hbox.pack_end(button, expand = False, fill = False)
        ...
        self.entry_for_ChooseName.connect("activate",
                                          self.SaveChooseNameWindow,
                                          old_name)
    ...
    def ScaleImage(self, pixbuf,
                   max_width = key_default_max_width_image,
                   max_height = key_default_max_height_image):
        h=pixbuf.get_height()
        w=pixbuf.get_width()
        scale = min(float(max_width)/float(w), float(max_height)/float(h))
        return pixbuf.scale_simple(dest_width = int(w*scale),
                                   dest_height = int(h*scale),
    ...
    def DeleteImageOfOldDiagrams(self,names):
        for name in names:
            ...
            if image_file == template_conf_image_no_diagram:
                continue
            try:
    ...
    def DeleteDiagramsFromLoadWindow(self, widget, data = None):
        ...
        if not result == gtk.RESPONSE_YES:
            msgbox.destroy()
            return
        names = [self.store[number][self.COL_NAMES]
    ...
    def CreateLoadTemplatesWindow(self):
        ...
        ### ICONS ###
        ## close button
        button = GetButtonFromStock(gtk.STOCK_CLOSE,
                                    'Close',
                                    self.DeleteLoadWindow)
        hbox.pack_end(button, expand = False, fill = False)

        ## load button
        button = GetButtonFromStock(gtk.STOCK_ADD,
                                    'Load',
                                    self.AddLoadWindow)
        hbox.pack_end(button, expand = False, fill = False)

        ## rename button
        button = GetButtonFromStock(gtk.STOCK_EDIT,
                                    'Rename',
                                    self.RenameLoadWindow)
        hbox.pack_end(button, expand = False, fill = False)

        ## remove button
        button = GetButtonFromStock(gtk.STOCK_DELETE,
                                    'Delete',
                                    self.DeleteDiagramsFromLoadWindow)
        hbox.pack_end(button, expand = False, fill = False)
        ...
        button = GetButtonFromStock(gtk.STOCK_DIRECTORY,
                                    'Restore',
                                    self.RestoreBackupConfigurationCallback)
        hbox.pack_start(button, expand = False, fill = False)

        ## create backup button
        button = GetButtonFromStock(gtk.STOCK_SAVE,
                                    'Create',
                                    self.CreateBackupConfiguration)
        hbox.pack_start(button, expand = False, fill = False)
        ...
        for name in self.diagrams.sections:
            ...
            changed = False
            if image_name == template_conf_image_no_diagram or \
            ...
            try:
                ...
                if changed == False:
                    # this means that the initial image_name exists,
                    # but it's broken, since we assume that the image
                    # for no diagram gives no error
                    self.CreateImageForDiagram(diagram,
                                               name = name,
                                               remove_temp = True)
                    image_name = diagram[key_diagram_image]
